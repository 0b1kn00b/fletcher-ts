...
function Handler(self) {
  return (r) => {
    Work.Submit(self.defer(r, Junction.Unit()));
  };
}
function Race(self, that) {
  return Anon2((p, cont) => {
    ...
    return Work.fromPromise(Promise.any([Resolve(a, p), Resolve(b, p)]).then((_) => deferred.promise.then((r) => cont.receive(Junction.issue(r)))));
  });
}
